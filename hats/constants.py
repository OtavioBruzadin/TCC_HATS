...
#
# Este pipeline reproduz o HATS.py do CRAAM bit a bit, e é essa a afirmação que
# ele sustenta: uma reimplementação independente, reorganizada e otimizada,
# chegando exatamente ao mesmo resultado do código original.
#
# Duas consequências, deliberadas:
#
#   - os registros anteriores à hora nominal são descartados, como na
#     referência, embora `sample` e `husec` sigam contínuos neles;
#
#   - a recursão de Goertzel encerra um passo antes do devido, reproduzindo o
#     defeito do windowed_dft.c, o que custa cerca de 0,003% na amplitude.
#
# As correções desses dois pontos estão no projeto irmão TCC_HATS_corrigido,
# separado justamente porque mudam o resultado.
#
# O que NÃO é sacrificado: as correções de unidade do apontamento, a marcação
# dos registros defasados, o reparo dos carimbos da estação meteorológica e a
# estatística de arquivo inteiro. Todas são aditivas — acrescentam colunas e
# campos sem alterar nenhum valor que a referência produz.
